<form method="POST" action="{{ route('offers.update', $offer->id) }}" class="realtime-validation">
    ...
    <div class="row mb-3">
        <div class="col-md-6">
            ...
            <select name="repeat_type" class="form-select" id="edit_repeat_type" required>
        ...
        <div class="col-md-6">
            <div class="mb-3" id="edit_repeat_until_container" style="{{ $offer->repeat_type === 'monthly' ? '' : 'display:none;' }}">
    ...
    <div class="mb-3">
        <label class="form-label">Delatnosti agencija koje vam odgovaraju (favorizuje ali ne isključuje ostale)</label>
        <input type="text" class="form-control mb-2" placeholder="Pretraži delatnosti..."
            oninput="filterOptions(this, 'edit-industry-options')">
        <div class="card border">
            <div class="card-body" style="max-height: 250px; overflow-y: auto;" id="edit-industry-options">
                @foreach($industries as $industry)
                <div class="form-check mb-1">
                    <input class="form-check-input" type="checkbox" name="industries[]" id="edit_industry_{{ $industry->id }}" value="{{ $industry->id }}"
                        {{ in_array($industry->id, $offer->industries->pluck('id')->toArray()) ? 'checked' : '' }}>
                    <label class="form-check-label" for="edit_industry_{{ $industry->id }}">
                        {{ $industry->name }}
                    </label>
                </div>
                @endforeach
